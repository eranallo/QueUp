...
export default function App() {
  ...
  const toggleFood = (id) => setTriedFood(prev => {
    const next = new Set(prev)
    next.has(id) ? next.delete(id) : next.add(id)
    saveSet('pp_tried_food', next)
    return next
  })
  const toggleDrink = (id) => setTriedDrinks(prev => {
    const next = new Set(prev)
    next.has(id) ? next.delete(id) : next.add(id)
    saveSet('pp_tried_drinks', next)
    return next
  })

  return (
    <AppContext.Provider value={{ checkedRides, toggleRide, foundMickeys, toggleMickey, triedFood, toggleFood, triedDrinks, toggleDrink }}>
